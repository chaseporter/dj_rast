...
bpm = 400
increment = int(song_samples)*float(bpm)/(float(song_length)*float(1000))
...
if mode == 'separate':
	...
	def updatefig(fig):
	    global a, avg, diff
	    a += increment
	    value = frequency_centroid[int(a)]
	    value_contrast = abs(avg_loud - loudness_spread[int(a)])/diff
	...
	pygame.init()
	song_to_play = pygame.mixer.Sound(song_str + '.wav')
	print "PLAYING MUSIC"
	song_to_play.play()
