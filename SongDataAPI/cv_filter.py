...
def blur(img, value) :
	...
	if (factor == 0) :
		factor = 1
	kernel = np.ones((int(factor),int(factor)),np.float32)/pow(factor, 2) # blur
	return cv2.filter2D(img, -1, kernel)
...
def contrast(img, alpha, beta) :
	...
	if len(size) == 3: 
		...
		for i in range(height): 
			for j in range(width):
				dst[i, j, 0] = dyn[img[i, j, 0]]
				dst[i, j, 1] = dyn[img[i, j, 1]]
				dst[i, j, 2] = dyn[img[i, j, 2]]
	else: 
