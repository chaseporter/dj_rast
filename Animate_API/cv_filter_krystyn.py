...
def edge_detect_faded(img, value, factor):
	vertical = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]) #  vertical gradient
	horizontal = np.array([[-1, -2, -2], [0, 0, 0], [1, 2, 2]]) # horizontal gradient
	if value == 0 or value == 1 or value == 2 or value == 6: # gray result
		gray = len(img.shape)
	...
	h_grad = np.array(abs_h, dtype=np.uint8)
	if value == 1 or value == 4:
		return v_grad * factor
	elif value == 2 or value == 5: 
		return h_grad * factor
	else: 	
		if value == 3: #color image
			height, width, x = img.shape
			dst = np.empty((height, width, 3), dtype=np.uint8)
...
def updatefig(fig):
    ...
    if a == 0: 
    	...
    	# im.set_array(v_grad)
    	# im.set_array(img_black)
    	# im.set_array(basic)
    	im.set_array(dst_faded)
    	im.set_cmap('Greys_r')
    	a = 0
    elif a == 2:
    	im.set_array(h_grad)
...
intervals = {}
intervals["fly_me_to_the_moon"] = 400
intervals["chum"] = 650
intervals["johnny_guitar"] = 675
intervals["this_game"] = 300 # was 275

ani = animation.FuncAnimation(fig, updatefig, interval=intervals["fly_me_to_the_moon"], blit=True)
